...
var Play = function (_React$Component) {
  ...
  function Play(props) {
    ...
    _this2.checkWin = function (userIndex, point) {
      ...
      _this2.checkExpandMap(point);
    };

    _this2.checkExpandMap = function (point) {
      ...
      if (point.x === x[0]) {
        ...
        var minX = x[0];
        x = [minX - 2, minX - 1].concat(_toConsumableArray(x));
      }
      // react the right
      if (point.x === x[x.length - 1]) {
        var maxX = x[x.length - 1];
        x = [].concat(_toConsumableArray(x), [maxX + 1, maxX + 2]);
      }
      // reached the top
      if (point.y === y[0]) {
        var minY = y[0];
        y = [minY - 2, minY - 1].concat(_toConsumableArray(y));
      }
      // reached the bottom
      if (point.y === y[y.length - 1]) {
        var maxY = y[y.length - 1];
        y = [].concat(_toConsumableArray(y), [maxY + 1, maxY + 2]);
      }
